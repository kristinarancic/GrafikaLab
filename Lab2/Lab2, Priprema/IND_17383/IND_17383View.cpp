...
void CIND17383View::DrawBranch(CDC* pDC, int width, int height, bool isLight)
{
	...
	//CRect fajl(5 * 25, 5 * 25, 10 * 25, 15 * 25);
...
void CIND17383View::DrawFigure(CDC* pDC)
{
	...
	Rotate(pDC, 3.14 * beta / 180, false);//veca ROTACIJA
	DrawBranch(pDC, 2 * 25, 3 * 25, true);//prva LIGHT grana
	...
	Rotate(pDC, 3.14 / 4, false);
	DrawBranch(pDC, 1 * 25, 3 * 25, false);//udesno izduzena grana

	pDC->SetWorldTransform(&saveForm); //vracam se na srednju izduzenu granu

	Translate(pDC, -2.1 * 25, -2.1 * 25, false);
	Rotate(pDC, -3.14 / 2, false);
	DrawBranch(pDC, 1.5 * 25, 3 * 25, false);//na levu stranu polozena ulevo

	pDC->GetWorldTransform(&saveForm1);//sacuvana pozicija od levu stranu, polozena ulevo grana
	Rotate(pDC, 3.14 / 2, false);
...
void CIND17383View::OnDraw(CDC* pDC)
{
	...
	//Translate(pDC, 5 * 25, 5 * 25, false);
	//Rotate(pDC, 3.14 / 2, false);

	//DrawBranch(pDC, 3 * 25, 5 * 25, false);
